...
class RoadDataLoader(NodeTrafficLoader):
    ...
    def __init__(self, dataset, city, threshold_road_distance, graph, with_lm=True, **kwargs):

        super(RoadDataLoader, self).__init__(dataset=dataset,
                                             city=city, graph=graph, with_lm=True, **kwargs)
            ...
            for graph_name in graph.split('-'):
                ...
                if graph_name.lower() == 'road_distance':
                    ...
                    adj_path = os.path.join(
                        os.getcwd(), "{}_{}_adjacency.tmp".format(dataset, city))
                    ...
                    if os.path.exists(adj_path):
                        ...
                        def extrLatLng(x): return x[2:8]
                        adjacency = PointlistToAdjacency(
                            [extrLatLng(x) for x in self.dataset.data['Node']['StationInfo']])
                    ...
                    if self.LM.shape[0] == 0:
                        self.LM = np.array(
                            [GraphBuilder.adjacent_to_laplacian(self.AM[-1])], dtype=np.float32)
                    else:
                        self.LM = np.vstack(
                            (self.LM, (GraphBuilder.adjacent_to_laplacian(self.AM[-1]))[np.newaxis, :]))
...
parser.add_argument('-p', '--update_params',
                    default='gcn_k:1,gclstm_layers:1,batch_size:16,mark:TEST')
...
if terminal_vars['decay_param'] is not None:
    terminal_vars['decay_param'] = os.path.join(
        os.getcwd(), terminal_vars['decay_param'])
...
# Config data loader
data_loader = RoadDataLoader(dataset=args['Dataset'], city=args['City'],
                             ...
                             threshold_road_distance=int(
                                 args['threshold_road_distance']),
