...
from tensorflow.python.ops.rnn_cell_impl import RNNCell, LSTMStateTuple, _concat

_BIAS_VARIABLE_NAME = "bias"
_WEIGHTS_VARIABLE_NAME = "kernel"


class GCLSTMCell(RNNCell):
    ...
    def build(self, inputs_shape):
        ...
        self._theta = self.add_variable(
            'ChebyshevParameter',
            shape=[self._num_layers, self._gcn_k + 1],
            initializer=self._initializer)
    ...
    def __call__(self, inputs, state, **kwargs):
        num_proj = self._num_units or self._num_proj
        ...
        for layer_index in range(self._num_layers):
            ...
            for i in range(0, self._gcn_k + 1):
                ...
                cheby_ply_x.append(self._theta[layer_index][i] * math_ops.matmul(T[-1], x0))
                cheby_ply_h.append(self._theta[layer_index][i] * math_ops.matmul(T[-1], h0))
            if layer_index > self._num_layers - 1:
                x0 = math_ops.tanh(math_ops.reduce_sum(cheby_ply_x, axis=0))
                h0 = math_ops.tanh(math_ops.reduce_sum(cheby_ply_h, axis=0))
            else:
                x0 = math_ops.reduce_sum(cheby_ply_x, axis=0)
                h0 = math_ops.reduce_sum(cheby_ply_h, axis=0)
        ...
        if self._use_peepholes:
            c = ( math_ops.sigmoid(f + self._forget_bias + self._w_f_diag * c_prev) * c_prev +
                  math_ops.sigmoid(i + self._w_i_diag * c_prev) * self._activation(j))
        else:
            c = ( math_ops.sigmoid(f + self._forget_bias) * c_prev +  math_ops.sigmoid(i) *
                 self._activation(j))
        ...
        if self._use_peepholes:
            m =  math_ops.sigmoid(o + self._w_o_diag * c) * self._activation(c)
        else:
