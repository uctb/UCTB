...
class DataSet(object):
    ...
    def merge_data(self, data, dataType):
        ...
        fragment_num = len(data)
        region_num = data[0].shape[1]
        timeslots_num = 0
        for index in range(fragment_num):
            timeslots_num += data[index].shape[0]

        # if data.shape[0][0] % self.MergeIndex is not 0:
        #     raise ValueError("time_slots % MergeIndex should be zero")

        if dataType.lower() == "node":
            new_array = []
            for i in range(fragment_num):
                new = np.zeros((data[i].shape[0] // self.MergeIndex, region_num), dtype=np.float32)
                for new_ind, ind in enumerate(range(0, data[i].shape[0], self.MergeIndex)):
                    new[new_ind, :] = func(data[i][ind:ind + self.MergeIndex, :], axis=0)
