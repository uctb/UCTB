...
with open("ARIMAresult3.txt","w") as fp:
    ...
    for index in tqdm(range(len(dataset))):
        ...
        f_tmp = os.popen("python -W ignore ARIMA.py --dataset {} --city {} --MergeIndex 1 --DataRange {} --TrainDays {} --MergeWay {} --test_ratio {}".format(dataset[index][0],dataset[index][1],dataset[index][2],dataset[index][3],dataset[index][4],dataset[index][5]), "r")
        # to record ouput
        fp.write(f_tmp.read()) 
        fp.flush()
        f_tmp.close()
