...
def save_predict_in_dataset(data_loader, predict_val, method, error=None):
    # 获得路径"UCTB/data"
    ...
    GroundTruth = data_loader.normalizer.min_max_denormal(data_loader.test_y)
    pred_data['Pred'][loader_id]["GroundTruth"] = np.squeeze(GroundTruth)
    ...
    if loader_id.endswith("N"):
        ...
        if error:
            pred_data['Pred'][loader_id][method]["rmse"] = error[0]
            pred_data['Pred'][loader_id][method]["mape"] = error[1]
            pred_data['Pred'][loader_id][method]["mae"] = error[2]
