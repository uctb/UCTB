    ...
    def build(self):
        with self._graph.as_default():
            ...
            for graph_index in range(self._num_graph):
                with tf.variable_scope('gc_lstm_%s' % graph_index, reuse=False):
                    ...
                    if type(self._gcn_k) is list:
                        gc_lstm = GCLSTMCell(self._gcn_k[graph_index], self._gcn_layer[graph_index], self._num_node,
                                             self._num_hidden_unit, state_is_tuple=True,
                                             initializer=tf.contrib.layers.xavier_initializer())
                    else:
                        gc_lstm = GCLSTMCell(self._gcn_k, self._gcn_layer, self._num_node,
                                             self._num_hidden_unit, state_is_tuple=True,
                                             initializer=tf.contrib.layers.xavier_initializer())
            ...
            if self._num_graph > 1:
                ...
                _, gal_output_name = GAL.add_ga_layer(graph=self._graph, inputs_name=tf.concat(outputs_last_list, axis=-2).name,
                                                      units=64, num_head=2, with_self_loop=True)
                gal_output = self._graph.get_tensor_by_name(gal_output_name)
                pre_input = tf.reshape(tf.reduce_mean(gal_output, axis=-2), [-1, self._num_node, 1, self._num_hidden_unit])
            else:
